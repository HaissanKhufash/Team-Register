const mongoose = require('mongoose'),
  ...
  uri = NODE_ENV !== 'production' ? MONGODB_URL_TEST : MONGODB_URL;
...
(async () => {
  try {
    ...
    const db = await mongoose.connect(uri, { useUnifiedTopology: true, useNewUrlParser: true });
    if (NODE_ENV !== 'test') console.log('Database conected', db.modelNames());
  } catch (err) {
